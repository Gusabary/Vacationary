...
int main()
{
    ...
    Shader lightingShader("shader/shader.vs", "shader/lightingShader.fs");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        lightingShader.setVec3f("light.ambient", ambientValue, ambientValue, ambientValue);
        lightingShader.setVec3f("light.diffuse", diffuseValue, diffuseValue, diffuseValue);
        lightingShader.setVec3f("light.specular", specularValue, specularValue, specularValue);

        // world transformation
        glm::mat4 model = glm::mat4(1.0f);
        lightingShader.setMatrix4fv("model", glm::value_ptr(model));
...
void processInput(GLFWwindow *window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_L) == GLFW_PRESS)
        specularValue -= 0.001;
    if (ambientValue > 1.0f)
        ambientValue = 1.0f;
    if (ambientValue < 0.0f)
        ambientValue = 0.0f;
    if (diffuseValue > 1.0f)
        diffuseValue = 1.0f;
    if (diffuseValue < 0.0f)
        diffuseValue = 0.0f;
    if (specularValue > 1.0f)
        specularValue = 1.0f;
